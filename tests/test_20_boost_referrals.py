...
class TestBoostComplet:
    ...
    def test_voir_prix_boost(self, auth_client):
        r = auth_client.get('/api/v1/boost/prices/')
        assert r.status_code == status.HTTP_200_OK

    def test_prix_bronze_correct(self):
    ...
    def test_agent_achete_boost_bronze(self, auth_agent):
        mock = {'success': True, 'reference': 'KPAY-BOOST-B001', 'status': 'pending', 'ussd_code': '', 'checkout_url': ''}
    ...
    def test_agent_achete_boost_silver(self, auth_agent):
        mock = {'success': True, 'reference': 'KPAY-BOOST-S001', 'status': 'pending', 'ussd_code': '', 'checkout_url': ''}
        with patch('apps.payments.services.kpay.kpay_service.collect', return_value=mock):
            r = auth_agent.post('/api/v1/boost/activate/', {
                'level': 'silver',
                'duration_days': 15,
    ...
    def test_agent_achete_boost_gold(self, auth_agent):
        mock = {'success': True, 'reference': 'KPAY-BOOST-G001', 'status': 'pending', 'ussd_code': '', 'checkout_url': ''}
        with patch('apps.payments.services.kpay.kpay_service.collect', return_value=mock):
            r = auth_agent.post('/api/v1/boost/activate/', {
                'level': 'gold',
                'duration_days': 30,
    ...
    def test_non_authentifie_bloque_boost(self, api_client):
        r = api_client.post('/api/v1/boost/activate/', {
            'level': 'bronze', 'duration_days': 7, 'target_city': 'Douala',
        })
...
class TestParrainage:
    ...
    def test_referral_code_genere_unique(self, client_user, create_user):
        user2 = create_user(email='ref_u2@test.cm', phone='+237670000070', role='client')
        user3 = create_user(email='ref_u3@test.cm', phone='+237670000071', role='client')
        codes = {client_user.referral_code, user2.referral_code, user3.referral_code}
        assert len(codes) == 3
        for code in codes:
            assert code is not None
            assert len(code) >= 6

    def test_inscription_avec_code_cree_referral(self, api_client, client_user):
    ...
    def test_non_authentifie_bloque_referral(self, api_client):
