...
class RequestVisitView(APIView):
    ...
    @extend_schema(tags=['Visits'], summary="Demander une visite", request=CreateVisitSerializer)
    def post(self, request):
        if request.user.role not in ['client', 'tenant']:
            return Response(error_response("Seuls les clients peuvent demander des visites"), status=status.HTTP_403_FORBIDDEN)
        ...
        try:
            prop = Property.objects.select_related('agent__agent_profile').get(id=property_id, status='available')
        except Property.DoesNotExist:
            return Response(error_response("Bien introuvable ou non disponible"), status=status.HTTP_404_NOT_FOUND)
        ...
        if not relation:
            return Response(error_response("Choisissez un agent d'abord"), status=status.HTTP_400_BAD_REQUEST)

        agent = relation.agent

        existing = VisitRequest.objects.filter(
            client=request.user, visit_property=prop, status__in=['pending', 'scheduled']
        ).exists()
        if existing:
            return Response(error_response("Vous avez déjà une demande en cours pour ce bien"), status=status.HTTP_400_BAD_REQUEST)

        visit_fee = 0
        is_free   = True
        if hasattr(agent, 'agent_profile') and not getattr(settings, 'FREE_MODE', True):
...
class ScheduleVisitView(APIView):
    ...
    def post(self, request, visit_id):
        try:
            visit = VisitRequest.objects.select_related('client', 'visit_property').get(
                id=visit_id, agent=request.user, status='pending'
            )
...
class ConfirmGPSView(APIView):
    ...
    @extend_schema(tags=['Visits'], summary="Confirmer présence GPS", request=GPSConfirmSerializer)
    def post(self, request, visit_id):
        try:
            visit = VisitRequest.objects.select_related('visit_property').get(
                id=visit_id, client=request.user, status='scheduled'
            )
        except VisitRequest.DoesNotExist:
            return Response(error_response("Visite introuvable"), status=status.HTTP_404_NOT_FOUND)
        ...
        if prop.location:
            within_radius, distance = is_within_radius(client_lat, client_lng, prop.location.y, prop.location.x)
            if not within_radius:
                return Response(
                    error_response(f"Vous êtes à {int(distance)}m du bien. Maximum: {settings.VISIT_GPS_RADIUS_METERS}m"),
...
class AgentConfirmVisitView(APIView):
    ...
    def post(self, request, visit_id):
        try:
            visit = VisitRequest.objects.select_related('client').get(
                id=visit_id, agent=request.user, status='scheduled'
            )
        except VisitRequest.DoesNotExist:
            return Response(error_response("Visite introuvable"), status=status.HTTP_404_NOT_FOUND)
        ...
        return Response(success_response(message="Visite confirmée ✅"))
...
class CancelVisitView(APIView):
    ...
    def post(self, request, visit_id):
        try:
            visit = VisitRequest.objects.select_related('client', 'agent').get(
                id=visit_id, status__in=['pending', 'scheduled']
            )
        except VisitRequest.DoesNotExist:
            return Response(error_response("Visite introuvable"), status=status.HTTP_404_NOT_FOUND)

        # IDOR protection — vérifier que l'utilisateur est bien client ou agent de cette visite
        if request.user.role != 'admin':
            if request.user.id not in [visit.client_id, visit.agent_id]:
...
class MyVisitsView(APIView):
    ...
    def get(self, request):
        if request.user.role in ['client', 'tenant']:
            visits = VisitRequest.objects.filter(client=request.user).select_related(
                'agent', 'visit_property'
            )
        elif request.user.role == 'agent':
            visits = VisitRequest.objects.filter(agent=request.user).select_related(
                'client', 'visit_property'
            )
        else:
            visits = VisitRequest.objects.none()

        paginator  = StandardResultsSetPagination()
        page_data  = paginator.paginate_queryset(visits.order_by('-created_at'), request)
        serializer = VisitRequestSerializer(page_data, many=True)
...
class LeaveReviewView(APIView):
    ...
    @extend_schema(tags=['Visits'], summary="Laisser un avis après GPS", request=VisitReviewSerializer)
    def post(self, request, visit_id):
        try:
            visit = VisitRequest.objects.select_related('agent', 'visit_property').get(
                id=visit_id, client=request.user,
                status='completed', client_gps_confirmed=True
            )
        except VisitRequest.DoesNotExist:
            return Response(error_response("Visite non éligible"), status=status.HTTP_404_NOT_FOUND)

        if hasattr(visit, 'review'):
            return Response(error_response("Avis déjà laissé"), status=status.HTTP_400_BAD_REQUEST)
        ...
        return Response(success_response(serializer.data, "Avis enregistré ✅"), status=status.HTTP_201_CREATED)
